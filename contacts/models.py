...
class ImportBatch(Main):
    ...
    name = models.CharField(max_length=255, help_text="User-defined import name, e.g. 'Q1 Trade Show Leads'")
...
class Contact(Main):
    ...
    additional_data = models.JSONField(default=dict, blank=True, help_text="Additional JSON attributes")
    import_batch = models.ForeignKey(
        ImportBatch, on_delete=models.CASCADE, null=True, blank=True,
        related_name='contacts', help_text="The import batch this contact belongs to"
    )
    source = models.CharField(max_length=255, blank=True, null=True, help_text="Import batch name, denormalized for fast filtering")
    ...
    def clean(self):
        ...
        if not self.email and not self.phone:
            raise ValidationError(_("Either Phone or Email must be provided alongside the Name."))
...
class ContactLog(Main):
    ...
    contact = models.ForeignKey(
        Contact, on_delete=models.CASCADE, related_name='logs'
    )
    crm = models.ForeignKey(
        'crm.CRM', on_delete=models.CASCADE, null=True, blank=True, related_name='logs'
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='contact_logs'
    )
...
class ContactRemark(Main):
    ...
    crm = models.ForeignKey(
        'crm.CRM', on_delete=models.CASCADE, null=True, blank=True, related_name='remarks'
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='contact_remarks'
    )
